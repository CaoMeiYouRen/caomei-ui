...
- [ ] Step 2.5: 按 audit-depth 分配审查深度 ⚠️ REQUIRED（控制用时）
    ...
    - [ ] 2.5.2 **轮次与范围**：轮次上限、各轮次输出约束、复审范围冻结与轮次未过时的改进协议均以 [AI 协作规范 §3.4 / §3.5](../../../docs/standards/ai-collaboration.md) 为唯一权威，本技能不重述轮次数值。
    - [ ] 2.5.3 证据优先采信：调用方提供的已查证事实（实验证据、测试结果、源码行号引用）直接采用，翻源码仅限需要最终实锤且无外部参考的场景。
    - [ ] 2.5.4 收敛策略（不依赖时间感知）：审查输出固定为"audit-depth 审查范围内可交付的结论 + 未覆盖边界"，宁可给 `Reject`（附待补证据清单）也不无限深挖。
    - [ ] 2.5.5 复审只审修复点：后续轮次只复查上轮问题编号对应的修复点 diff 与受影响断言，不得重读全量 diff（轮次范围与冻结条件见 [AI 协作规范 §3.4](../../../docs/standards/ai-collaboration.md)）。
    - [ ] 2.5.6 并发分区（仅大改动，触发条件与汇总规则见 [AI 协作规范 §3.2](../../../docs/standards/ai-collaboration.md)）：被分区的任务只对本分区范围独立出结论。
    - [ ] 2.5.7 用时反馈由调用方事后实测：审计方不自报时长、不检查时间。
...
- [ ] Step 4: 收集并延续审查证据
    ...
    - [ ] 4.2 按 [evidence-template.md](./references/evidence-template.md) 预填与延续：首轮创建，多轮 review 复用同一份记录，按轮次顺序追加章节，保留未关闭问题编号（RG-B/W/S 系列）与复查结论。
- [ ] Step 5: 进行结构化审查
...
- [ ] Step 6: 判定严重级别 ⚠️ REQUIRED
    ...
    - [ ] 6.5 **复发属性**：每条 finding 标注「首次」或「复发（上轮编号）」；复发判定门槛见 [AI 协作规范 §3.5](../../../docs/standards/ai-collaboration.md)，复发类 finding 的修复方向必须含约束载体（治理脚本 / 测试 / 规范必查项），不得只给当轮改法。
...
- 超限 diff 未核对拆分依据就放行。
- 复发类问题只给当轮改法，不给出防复发载体（脚本 / 测试 / 规范必查项）。
- 复审时重述权威文档的轮次 / 时间盒数值，而非链接引用。
...
- [ ] 每条 finding 已标注首次 / 复发属性；复发类 finding 已给出约束载体（脚本 / 测试 / 规范必查项）。
