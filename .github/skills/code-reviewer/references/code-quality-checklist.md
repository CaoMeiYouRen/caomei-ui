...
审查新增/修改的注释与测试名是否残留规划/任务/审计/backlog 编号标记；编号禁令、例外与修复方式见 [规划规范 §4](../../../../docs/standards/planning.md#4-阶段与条目命名)，本清单不重述。
...
改动含批量替换、脚本改写或大段删除时，检查目标文档是否**完整**：

- **核验命令**：`pnpm docs:check:integrity`（0 字节 / 纯空白 / 代码围栏失衡 / 孤立表格行，均不依赖 HEAD；**缩减告警依赖「工作区 vs HEAD」，洁净检出 / 已提交状态下不触发**）；逐个 `git show :<file> | wc -l`；`git diff --stat` 的删除量是否与声明的清理意图一致。
- **判据与成因**（门禁盲区与结构性破坏的具体形态）：见 [AI 协作规范 §8](../../../../docs/standards/ai-collaboration.md) 与本 skill 的 Step 5.9，本清单不重述。**核心提醒**：空 `.md` 对 `lint-md` / `check-links` / `check-line-count` 全部放行，不能以「doc 门禁通过」作为文档未被损坏的证据。
- **判级**：文件被写空 / 被截断而改动声明未提及时判 `blocker`；缩减告警（未达 30%）需调用方说明是否为有意清理，未说明记 `warning`。
...
- **挂接声明**：严格约束是否在规范文档中声明 review 挂钩（如 planning.md §3"合规核验由 review 阶段执行"句式）？
