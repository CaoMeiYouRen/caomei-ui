...
### 最低验证要求
- 目标层级:（对照 code-reviewer SKILL.md 最低验证矩阵）
- 需要的命令:
...
- 结论: Pass | Reject
- 失败原因或通过条件:
- 实际用时 / 是否超时间盒:（调用方宿主时钟事后实测回填，agent 自报用时为估算值不作数）
- 本轮新增问题:
- 本轮已关闭问题:
- 待复查问题:
...
- 复审轮次只复查上轮未关闭问题编号对应的修复点 diff，不重读全量 diff（轮次范围与冻结条件见 [AI 协作规范 §3.4](../../../../docs/standards/ai-collaboration.md)）。
