...
- [ ] Step 3: 响应式 ⚠️ REQUIRED
  ...
  - [ ] 3.2 按[响应式设计 §3](../../../docs/design/responsive.md) 的窄屏行为矩阵核对窄屏行为（组件自身 chrome 收敛：宽度内收 / 换行 / 横向滚动 / 截断）与长文本溢出 / 截断；页面级侧栏 / 抽屉的窄屏形态见 Step 1 的页面级检查。卡片化与转全屏不作为默认预期（仅当矩阵或组件文档如此声明时才断言）。
