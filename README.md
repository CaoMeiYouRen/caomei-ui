...
Tier 0 / Tier 1 / Tier 2 组件均已完成并归档；Phase 4（Tier 3 稳定批组件）已由用户决策启动。文档站增强、首版发布与下游接入（Phase 5+）待后续规划决策。组件库尚未发布首个版本，API 与目录结构在 1.0 前可能调整。

- 定位：替代多个下游项目中的 PrimeVue，规避 PrimeUI 商业许可风险。
- 目标组件集：Tier 0（9 个，已完成）+ Tier 1（8 个，已完成）+ Tier 2（6 个，已完成）+ Tier 3 稳定批（7 个，进行中）。
