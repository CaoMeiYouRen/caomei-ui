...
组件库已完成 Tier 0 ~ Tier 3 稳定批组件与各阶段补全：首个正式版本 **0.1.0** 于 2026-09-19 发布，当前最新版本 **0.2.0** 于 2026-09-22 发布到 npm（`latest`）。**0.2.0 含破坏性包形态变更**：样式入口由 `caomei-ui/styles.css` 改为 `caomei-ui/theme.css`（下游修复指引见[发布指南](./docs/guide/release.md#9-包形态与兼容性)）。Phase 12（发布就绪、文档对外与一致性收官）已完成并归档，当前无进行中阶段。采用本地手动发布，CI 自动发布暂缓，下游接入验证由发布后的实际迁移反馈驱动。0.x 期间 API 与目录结构仍可能调整。
...
当前最新版本为 `0.2.0`（样式入口为 `caomei-ui/theme.css`），组件库以 Vue 3.5+ 作为 peer 依赖（需在项目中自行安装）。0.x 阶段 API 与目录结构在 1.0 前可能调整。
