...
| 译文生成与复核流程 | 由 AI 基于 zh-CN 生成各语种译文；文件头部以注释标注「**AI 生成、待人工复核**」；守卫脚本只校验**键集合一致性**（不校验译文质量）；复核完成后移除标注，**复核结论落到本记录 §9 后续行**（或 `docs/components/locale.md` 的语种说明）。**发布前检查**：`pnpm build` 产物与 `src/locale/` 不得残留「待人工复核」标注（该判定为发布前触发项） |
