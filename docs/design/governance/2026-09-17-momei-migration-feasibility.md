...
**用户决策（2026-09-17）**：按 **C3 分批全量**执行；**先做 B1 库侧补齐**；B 级 **14 项全部完成**；**接受** 16 条有意差异；回归强度由**每周回归任务跑 momei 的测试**承载（不要求每批跑 momei 全量 E2E）。详见 §7。
...
| 3 | B 级增强取舍 | **14 项全部完成**（全部纳入 B1 执行，**不再分批取舍**，含评估曾建议延后的 4 项） |
...
| 载体 | 变更 |
| --- | --- |
| [待办事项](../../plan/todo.md) | 决策结论经再评估点段落登记；**2026-09-17 阶段归档清理**后，`todo.md` 不再承载未启动阶段的范围（Phase 10 已归档、当前无进行中阶段），故 B0~B4 的执行范围以[路线图 Phase 7 第二阶段](../../plan/roadmap.md)与[本记录 §6](./2026-09-17-momei-migration-feasibility.md) 为准 |
| [Backlog §1.8](../../plan/backlog.md) | 「momei 迁移执行」与「库侧增强清单（B1）」两行由「待决策」改为「已决策（2026-09-17）：按 C3 执行、B1 先行、B 级 14 项全做」 |
| [路线图](../../plan/roadmap.md) | Phase 7 第二阶段说明补「范围已由用户决策登记」与指针 |
| [治理索引](./index.md) | 该记录条目补「用户决策（2026-09-17，5 项）」摘要，范围指针改指[路线图 Phase 7 第二阶段](../../plan/roadmap.md)与本记录 §6（原指向 todo「下一阶段」段，随归档清理调整） |

**未决 / 待办（执行前）**：

- B0 / B1 的**原子条目拆分与开工顺序**（按[规划规范 §4](../../standards/planning.md)：一个条目 = 一个可独立验收、可独立提交的逻辑变更）；B1 的 14 项需按「依赖关系 + 影响面」排序（建议 A 级 DataTable 先行，其余按用量从高到低）。
