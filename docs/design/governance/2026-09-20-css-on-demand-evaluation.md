...
> **实现形态更新（2026-09-20）**：本节 7.1 决策之后，M1-1 构建路径 POC 实测表明**无需逐组件样式入口**——`unbundle: true` + `css.inject: true` 即可让消费方 tree-shaking 达成逐组件按需，依赖闭包问题随之消失。故本节下方表格中的「`components/<name>.css` 入口」「已拆组件从聚合入口移出」与 §7.3 的「集合不变量」验收标准**让渡权威**给 [M1-1 构建路径 POC](./2026-09-20-m1-1-build-path-poc.md) §3 / §7 与 [待办事项](../../plan/todo.md) 的 M1 节（2026-09-20 用户已确认该调整）；本节保留为决策时点的形态快照。§7.1 中**仍有效**的决策为 D2（不做逐组件入口）与 D3（兼容性不作要求）；**D1（暂不启动）、D4（与下游时序）与 D5（载体：待新需求评估再进 `todo.md`）已被 2026-09-20 的启动决策取代**——Phase 11 已授权启动并登记，现行状态以[待办事项](../../plan/todo.md) 与[路线图](../../plan/roadmap.md)为准。
...
> 批次 1 与批次 2 相加即为「小成本高收益」范围：全局层（theme 3.11 KB）+ 两个预设（1.98 / 1.90 KB）≈ **7 KB raw / ≈ 2.5 KB gzip**，重量级 7 组件 ≈ 38.7 KB raw；合计约占全量样式的 1/4，且**不触碰依赖闭包**（风险最低）。

### 7.3 调整后的最小验收标准（**快照：第 1~4 条已随实现形态更新让渡**）

> 本节为 2026-09-20 决策时点的验收标准快照。M1-1 POC 后，第 1 条（集合不变量）、第 2 条（聚合入口降幅与移出量一致）、第 3 条（7 个组件各自入口独立加载）、第 4 条（依赖闭包成立）**均已失效**（不再有逐组件入口与「移出」动作）；仍有效的第 5 条（门禁全绿）与第 6 条（文档口径同步）**其口径以 [M1-1 POC §7 A4](./2026-09-20-m1-1-build-path-poc.md) 与 [待办事项](../../plan/todo.md) 的 M1 节为准**（文档清单同见 A4）。
...
2026-09-20（出证时点快照）：本条需求经评估判定**未命中插队例外**，按 [规划规范 §3](../../standards/planning.md) 登记 [Backlog §1.6](../../plan/backlog.md)；同日用户 5 项裁定落定（§7.1），形态收敛为**分层拆分（非逐组件入口）**并给出批次清单（§7.2）与验收标准（§7.3）。**后续更新（2026-09-20）：该「未启动」状态已被取代**——用户已授权启动并登记为 Phase 11，构建路径 POC 已交付（结论「条件性可行」，推荐 `unbundle: true` + `css.inject: true`），实现形态改为**不做逐组件入口**（§7.2 的形态细节与 §7.3 的第 1~4 条验收标准已让渡）。**现行状态以[待办事项](../../plan/todo.md) 与[路线图](../../plan/roadmap.md)为准**，形态依据见 [M1-1 构建路径 POC](./2026-09-20-m1-1-build-path-poc.md)。本记录为现状与方案的取证依据，不构成实施承诺。
