...
- **未分配阶段编号**：按[规划规范 §4](../../standards/planning.md)，编号只在用户授权启动时分配（取已有最大编号 +1，即 Phase 13）。**（本节为产出时点状态；决策与登记结果见 §11，以 §11 为准。）**
...
## 11. 决策结果与登记结果（2026-09-23）

### 11.1 用户裁定

| 编号 | 事项 | 裁定 | 落点 |
| :-: | :--- | :--- | :--- |
| D1 | 主线取舍 | **M1 + M2 + M3 + M4**（全取） | §6 M1~M4 全部登记 |
| D2 | `DataTable` 新能力 API 命名取向 | **① 对齐 PrimeVue 命名**（`rowGroupMode` / `groupRowsBy` / `expandableRowGroups` / `expandedRowGroups` / `#groupheader` / `expandedRows` / `expander` / `#expansion` / `rowExpand` / `sortMode` / `multiSortMeta`） | M1 各条目的 API 契约；下游改写量与 e2e 重写面最小 |
| D3 | 分组 / 展开受控模型 | **两者都支持**（受控优先、缺省自持） | M1-2 / M1-3 |
| D4 | 降序优先配置 | **提供** `sortDescFirst` | M1-4 |
| D5 | `TagsInput` 形态 | **封装 Reka `TagsInput` primitive** | M2-1 |
| D6 | `Sidebar` 独立组件 | **不补**（维持 `CaomeiDrawer` 承接） | 不纳入表 |
| D7 | 是否在本阶段内发布 0.3.0 | **发布**（依赖 npm 凭据，本地手动发布） | M3-2 |
| D8 | Phase 8 是否随接入启动 | **维持未启动**（等 dependfix 完成迁移后再评估） | 不纳入表 |
| D9 | M4 治理守卫取舍 | **纳入 1 项**：文档站双花括号插值机检守卫 | M4-1 |

### 11.2 登记结果

- **阶段编号已分配**：**Phase 13（组件能力补齐与 dependfix 迁移解阻）**——按[规划规范 §4](../../standards/planning.md) 取已有最大编号 +1；用户裁定 D1~D9 构成该规范 §3.3 要求的「需求评估 + 用户明确决策」，登记不构成实施启动（实施以「开工」指令为准）。
- **`todo.md` 当前阶段已登记**：4 条主线 / **11 条原子条目**（M1 4 / M2 3 / M3 3 / M4 1）+ 各主线执行范围 / 非目标 / 最小验收标准 + 不纳入表；状态口径为「**登记 = 范围授权，实施未启动**」。
- **`roadmap.md`**：阶段表新增 Phase 13 行、§1 现状句、规划纪律注与状态行同步。
- **`backlog.md` 迁出标注**：`DataTable` 行分组与行展开 → M1、`DataTable` 多列排序与默认排序方向 → M1、`TagsInput`（标签录入）→ M2、文档站双花括号插值的机检守卫 → M4；T4 的 3 项条件候选与 T5 的 31 项长期候选**维持留 Backlog**。
- **本记录**：§11 为决策与登记结果的权威落点；§1~§10 为决策前快照（排序建议与 D 项候选取向以本节为准）。
