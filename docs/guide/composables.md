...
除组件外，caomei-ui 导出三个组合式函数（composables），用于主题、轻提示与确认对话框。经 Nuxt 模块接入时它们会被自动导入，其他项目从包入口显式导入。
...
服务式 composable 的状态由 Provider 实例持有，而非模块级单例；Provider 与调用方同层时会抛出明确错误。SSR 隔离的设计权衡见[组件设计 §7](/design/components)。
