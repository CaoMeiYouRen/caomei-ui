...
> **The authoritative per-component mapping is [Design spec §7](/design/design-spec.md) (Chinese)** — this page covers the workflow, shared conventions and pitfalls only, and does not duplicate the mapping tables. Component pages carry a "Migration from PrimeVue" section at the end of the page (placement rule in [Documentation site §4](/design/documentation-site.md), Chinese).
...
2. **Theme and tokens**: map the PrimeVue theme variables to this library's tokens (`--p-*` → `--caomei-*`) before replacing components; see [Theming and styles §4](/design/theming.md) (Chinese) for token semantics and override recipes. `caomei-ui/styles.css` ships the default light/dark themes and `data-preset` switches presets.
3. **Component replacement**: work through the mapping table in [Design spec §7](/design/design-spec.md) (Chinese), starting with the three highest-impact differences — controlled-field naming, icon form, and tone/size tiers (see the next section).
...
5. **Page-by-page verification**: after each page, check interaction, accessible names, narrow-screen behavior and dark mode; the narrow-screen acceptance criteria are in [Responsive design §4](/design/responsive.md) (Chinese).
...
> The table above lists **shared** reminders. Whether a given component supports something, and whether a difference is intentional, is always decided by that component's entry in [Design spec §7](/design/design-spec.md) (Chinese).

## Per-component index

Every component in the table below has a registered mapping in [Design spec §7](/design/design-spec.md) (Chinese), grouped as in the component sidebar; each name links straight to its component page, whose "Migration from PrimeVue" section (at the **end of the page**, before `API`) holds the mapping table.
...
- [Design spec §7: migration mapping](/design/design-spec.md) (Chinese; the authoritative per-component mapping)
- [Theming and styles](/design/theming.md) (Chinese; tokens and override recipes)
- [Responsive design](/design/responsive.md) (Chinese; narrow-screen acceptance criteria)
