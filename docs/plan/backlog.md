...
| DropdownMenuTrigger 样式豁免 | `CaomeiDropdownMenuTrigger` 把 `caomei-dropdown-menu__trigger` 默认外观固定在根上，`as-child` 组合自定义按钮（如 SplitButton 的下拉按钮）时会连带合并 padding / border / background；候选补 `unstyled` / 样式豁免入口，使复合层可复用其 a11y 接线而不继承默认外观（当前 SplitButton 直接用 Reka `DropdownMenuTrigger` 绕过）。**2026-09-18 进展**：同类 `CaomeiPopoverTrigger` 已按此模式补 `unstyled`（M5-4，声明式锚点迁移需要）；Popover 侧 `date-picker.vue` / `color-picker.vue` 仍是「直接用 Reka `PopoverTrigger as-child`」的绕过，候选在不动其现有外观的前提下收敛到 `unstyled`；DropdownMenu 侧仍未实现 | 低 |
