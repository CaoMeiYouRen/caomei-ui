...
| 移动端测试用例 | 用户需求（2026-09-14） | 为小屏适配补单元断言与 Playwright 多视口（mobile / tablet）回归；沿用现有测试规范与后续 E2E 规划。**第一步已落地（2026-09-17）**：根 `playwright.config.ts` + `test/e2e/**`（夹具 / helpers / 多视口用例），`pnpm test:e2e` 全绿；键盘聚焦口径已由用户裁定为分档判定（见下行候选与[待办事项](./todo.md) 条目说明）；剩余批次 3（DatePicker / Calendar）用例与改动前基线归档 | → M2（Phase 10 执行中） |
