...
| AutoComplete 严格选项模式 | M3 条目 2 迁移评估 | AutoComplete 在回车 / 失焦时会提交自由文本（`commitFreeText`），与 PrimeVue `Select filter`「值必须来自选项列表」的语义有差；候选补 `strict` / 限制自由文本的开关，或按 Reka `Listbox` 另立可搜索单选形态。触发条件：下游迁移实测出现「取值必须受限于选项列表」的受控字段用例。**2026-09-20 M2-1 判定：用量达标但需用户裁定**——momei @ `47cad194` 有 1 处受控字段（`translationId`，值须指向已有文章、下游未用 `forceSelection`）；新增 props 属契约变更，M2-1 建议**暂不纳入**（自由文本为有意设计，差异已登记[设计规范 §7](../design/design-spec.md)）。**待用户裁定**；触发条件细化为「下游明确要求限制取值 / 迁移实测暴露脏引用」。→ M2-1 已复核（Phase 11 组件样式按需化与能力增强） | 低 |
