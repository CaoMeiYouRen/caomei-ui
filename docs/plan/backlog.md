...
| 迁移入口表 ↔ 设计规范 §7 一致性 | 《从 PrimeVue 迁移》专题页的「逐组件对照入口」表需与 §7 已登记组件集合一致（2026-09-18 M6-7 首轮 Review 因漏 Dialog / Drawer 判 blocker；该页已声明「不宣称穷尽」并随 §7 维护）。候选：脚本机检两侧集合差并接入 `docs:check` | 低 |
