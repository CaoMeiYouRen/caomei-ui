...
| ColorPicker 色板导航增强 | M4 条目 5 follow-up | 色板当前为 `role="group"` + `aria-pressed` 按钮组（Tab 遍历，无方向键 roving）；候选改为 `radiogroup` + `aria-checked` 并补 roving tabindex。触发条件：下游启用 `swatches` 且出现键盘密集使用场景。**2026-09-20**：由 Phase 11 M2-1 按该主线的可复验门槛复核（下游实测用量 ≥1 处 + 不引入未登记契约变更）；**达标后须提交用户确认才纳入**。→ M2-1（Phase 11 组件样式按需化与能力增强） | 低 |
| AutoComplete 严格选项模式 | M3 条目 2 迁移评估 | AutoComplete 在回车 / 失焦时会提交自由文本（`commitFreeText`），与 PrimeVue `Select filter`「值必须来自选项列表」的语义有差；候选补 `strict` / 限制自由文本的开关，或按 Reka `Listbox` 另立可搜索单选形态。触发条件：下游迁移实测出现「取值必须受限于选项列表」的受控字段用例。**2026-09-20**：由 Phase 11 M2-1 按该主线的可复验门槛复核（下游实测用量 ≥1 处 + 不引入未登记契约变更）；**达标后须提交用户确认才纳入**。→ M2-1（Phase 11 组件样式按需化与能力增强） | 低 |
...
| AI 资产指针（`AGENTS.md`） | ① `AGENTS.md` §11「相关文档」缺长期任务台账与规划载体的指针；② **`AGENTS.md` 第 57 行的接入口径仍列 `caomei-ui/styles.css`**（Phase 11 M1-3 已改为 `caomei-ui/theme.css`，属**发布前必须闭环**的对外事实失真）。该文件为受保护文件，须用户明确指示后随一次授权变更执行（改 `.github/**` 时同步 `.claude/` `.opencode/` `.agents/` 镜像） | 低 |
