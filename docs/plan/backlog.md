...
| CSS 按需引入（分层拆分：基础层 + 重量级组件） | 用户需求（2026-09-20）。现状：`dist/styles.css` 单文件（167,585 字节 / gzip 25.60 KB）承载 tokens + 2 个预设 + 全量组件样式；`exports` 仅有 `./styles.css`；resolver 固定注入整份 → JS 侧可 tree-shaking、CSS 侧全量。**用户 5 项裁定（2026-09-20）**：① 暂不启动（不紧急，全量尚可接受）；② **不做逐组件入口**，先拆全局 `theme.css` + 预设 `presets/*.css` + 「最重且无上游依赖」的 7 个组件（auto-complete / drawer / multi-select / stepper / toolbar / file-upload / color-picker），依赖闭包类（select / button / input / calendar 家族）延后；③ 兼容性**不作要求**，下游自行修复；④ 启动时点原定「由下一次需求评估决定」，**2026-09-20 用户已授权启动并登记为 Phase 11（见行末标注）**。前置：构建路径 POC（`unbundle` vs 多入口 + `css.splitting`）。取证、批次清单与验收标准见[评估记录 §7](../design/governance/2026-09-20-css-on-demand-evaluation.md) → M1-1 ~ M1-4（Phase 11 组件样式按需化与能力增强） | 低 |
