...
| 候选 | 来源 | 说明 | 优先级 |
|------|------|------|--------|
| Tag/Badge 增强 | 组件实现评估 + M1 复核 | Tag 可选中筛选 / 可编辑（Reka TagsInput）；Badge 叠加位置偏移与宽度过渡动画 → M3-2（Phase 11 组件样式按需化与能力增强） | 中 |
| Select 增强 | 组件实现评估 + M1 复核 | 分组（SelectGroup）与可编辑组合。**`filter` 不在 Select 实现**：Reka Select 把 `role="listbox"` 固定在面板元素上（`SelectContentImpl` 在 `$attrs` 之后写入，无法覆盖），面板内放搜索框会使 textbox 成为 listbox 的 owned child（违反 WAI-ARIA `aria-required-children`），可搜索单选应走 `Listbox` / `Combobox` → M3-1（Phase 11 组件样式按需化与能力增强） | 中 |
| 实底前景 token 配对复核 | M3 复核 | 复核其余实底消费点的 `-solid` × `-foreground` 配对，避免跨主题配对冲突（规范口径见 [设计规范 §2.2 / §3.2](../design/design-spec.md)） | 中 |
| ColorPicker 色板导航增强 | M4 条目 5 follow-up | 色板当前为 `role="group"` + `aria-pressed` 按钮组（Tab 遍历，无方向键 roving）；候选改为 `radiogroup` + `aria-checked` 并补 roving tabindex。触发条件：下游启用 `swatches` 且出现键盘密集使用场景。**2026-09-20 M2-1 判定：不达标**——momei @ `47cad194` 中 `swatches` **0 命中**；键盘事件 **25 行 / 9 文件**（`git grep -nE "keydown|keyup|keypress"`，与色板相关 0），命令与口径见 [M2-1 盘点记录 §3.5](../design/governance/2026-09-20-m2-1-component-quality-audit.md)；触发条件据实维持。→ M2-1 已复核（Phase 11 组件样式按需化与能力增强） | 低 |
| AutoComplete 严格选项模式 | M3 条目 2 迁移评估 | AutoComplete 在回车 / 失焦时会提交自由文本（`commitFreeText`），与 PrimeVue `Select filter`「值必须来自选项列表」的语义有差；候选补 `strict` / 限制自由文本的开关，或按 Reka `Listbox` 另立可搜索单选形态。触发条件：下游迁移实测出现「取值必须受限于选项列表」的受控字段用例。**2026-09-20 M2-1 判定：用量达标；同日用户裁定「纳入严格选项模式」** → 登记为 [待办事项](./todo.md) **M3-5**（Phase 11 组件能力增强）。判别依据：momei @ `47cad194` 的 `translationId` 受控字段（值须指向已有文章、下游未用 `forceSelection`，命令见[M2-1 盘点记录 §2.1](../design/governance/2026-09-20-m2-1-component-quality-audit.md)）。由 M3-5 承载，本表不再作为待决策候选 | 低 |
...
| 候选 | 说明 | 优先级 |
|------|------|--------|
| 组件覆盖率门禁 | **范围修正（2026-09-20 用户裁定）**：覆盖率**不进入日常 `test` / `verify` 门禁**（现状约 90%，日常卡收益低），改为**仅在周期性回归任务与 release 门禁中校验**；阈值取实测基线 → M4-1（Phase 11 组件样式按需化与能力增强） | 中 |
| CSS 按需引入（分层拆分：基础层 + 重量级组件） | 用户需求（2026-09-20）。现状（**Phase 11 启动时**，形态已由 M1-1~M1-3 落地改变）：`dist/styles.css` 单文件（167,585 字节 / gzip 25.60 KB）承载 tokens + 2 个预设 + 全量组件样式；`exports` 仅有 `./styles.css`；resolver 固定注入整份 → JS 侧可 tree-shaking、CSS 侧全量。**用户 5 项裁定（2026-09-20）**：① 暂不启动（不紧急，全量尚可接受）；② **不做逐组件入口**，先拆全局 `theme.css` + 预设 `presets/*.css` + 「最重且无上游依赖」的 7 个组件（auto-complete / drawer / multi-select / stepper / toolbar / file-upload / color-picker），依赖闭包类（select / button / input / calendar 家族）延后；③ 兼容性**不作要求**，下游自行修复；④ 启动时点原定「由下一次需求评估决定」，**2026-09-20 用户已授权启动并登记为 Phase 11（见行末标注）**。前置：构建路径 POC（`unbundle` vs 多入口 + `css.splitting`）。**实现形态已由 POC 更新（2026-09-20 用户确认）**：改为 `unbundle: true` + `css.inject: true`（消费方 tree-shaking 按需），不再需要逐组件样式入口与依赖闭包批次；取证见[评估记录 §7](../design/governance/2026-09-20-css-on-demand-evaluation.md)（其中 §7.2 批次清单的实现细节以 [M1-1 POC 记录](../design/governance/2026-09-20-m1-1-build-path-poc.md) 为准）→ M1-1 ~ M1-3（Phase 11 组件样式按需化与能力增强） | 低 |
| AI 资产指针（`AGENTS.md`） | ① `AGENTS.md` §11「相关文档」缺长期任务台账与规划载体的指针；② ~~`AGENTS.md` 第 57 行的接入口径仍列 `caomei-ui/styles.css`~~ **已闭环（2026-09-20 用户授权后同步为 `caomei-ui/theme.css`）**。该文件为受保护文件，须用户明确指示后随一次授权变更执行（改 `.github/**` 时同步 `.claude/` `.opencode/` `.agents/` 镜像） | 低 |
| 文档翻译旧目录守卫 | 设计文档已声明「不保留 `docs/<locale>/`」但无自动校验；对齐 momei 增加 `docs:check:i18n`，检测旧目录回流与重复翻译页 → M4-2（Phase 11 组件样式按需化与能力增强） | 中 |
...
| 层级与阴影 token | 浮层组件 z-index 目前仍为字面量，后续抽 `--caomei-z-*` 统一管理（阴影 / 遮罩 token 已于 2026-09-17 第 5 轮落地，见[设计规范 §2.6](../design/design-spec.md)） → M2-3（Phase 11 组件样式按需化与能力增强） | 低 |
| scoped 变量声明治理 | `development.md §7` 要求「基类不预声明 CSS 变量默认值、档位类用 `:where()`」，但仍有偏差：`button` 基类直接声明 `--caomei-button-*` 默认值（基类预声明），`message` / `badge` / `tag` / `toast` 的变体类用普通类声明变量（档位类未用 `:where()`）；建议补 `check-design.mjs` 规则（现规则只查引用存在性）并逐组件收敛 → M2-2（Phase 11 组件样式按需化与能力增强） | 低 |
...
| 禁用态不透明度字面量回归守卫 | 2026-09-17 第 5 轮收敛了组件内 `opacity: 0.6`、第 6 轮收敛了 `opacity: 0.5`，两档合计 37 处已全部并入 `--caomei-disabled-opacity`（组件内 `src/components/**`），但 `check:design` 只守 `#hex` / `rgb()` / `hsl()`，无法阻止字面量回流。候选：为该属性加同类预算守卫（覆盖 `0.5` / `0.6`），避免一次性清理被后续开发稀释 → M2-3（Phase 11 组件样式按需化与能力增强） | 低 |
...
| 样式档位死声明回归守护 | 组件中 `:where()` 档位块直接声明属性（padding / font-size 等）会被更高特异性规则覆盖而静默失效，ToggleButton / Checkbox / RadioGroup 已各出现一次；建议对构建产物 CSS 加断言或补计算样式 E2E，并统一「档位只声明 CSS 变量」约定 → M2-2（Phase 11 组件样式按需化与能力增强） | 低 |
| 重复声明（死声明）守卫 + 同类残留清理 | 2026-09-20 M2-2 实现期发现（Review Gate W2）：D1 的「同规则内先写 token 色阴影、后写 `color-mix` 恒被覆盖」在 `input` / `input-number` / `select` / `textarea` **另有 8 处 / 4 组件**（HEAD 即已存在，非本批引入）。候选：① 清理这 8 处；② 补一条机检规则（同一规则内同名属性重复且后者覆盖前者 → error）并带正反例语料 | 中 |
| 等价验证产物入库 | 2026-09-20 M2-2 审查 S2：226 项计算样式等价矩阵的脚本与快照（`capture.mjs` / `capture-geometry.mjs` / `diff.mjs` / `baseline.frozen.json` / `after.json`）位于 gitignored 的 `.temp/capture/`，**等价结论不可从仓库直接复算**（基线 worktree 在 `/tmp`）。候选：把采样脚本 + 冻结基线入库（约 156 KB）或建立可复现的等价验证脚本入口 | 低 |
| 尺寸档位类未 `:where()` 归一化（28 条 / 8 组件） | 2026-09-20 M2-2 实现期发现（Review Gate W2）：`.caomei-<comp>--sm\|md\|lg` 以**普通类**直接声明几何属性的规则共 **28 条**——input-number 6、textarea 3、tag 3、select 3、input 3、date-picker 3、button 3、badge 4（3 单一 + 1 条 `--dot` × `--lg` 复合块）。它们**当前生效**（非可见缺陷），偏差在于未按 [开发规范 §7](../standards/development.md)「档位类用 `:where()` 归零特异性 + 档位只声明 CSS 变量」归一化，故消费方覆盖需与 (0,2,0) 特异性竞争。G1 / G2 的机检面（限 `:where()` 块与 `--caomei-*` 声明）**不拦截**该类，属登记在案的残余风险（见[落地记录 §6](../design/governance/2026-09-20-m2-2-m2-3-style-governance-landing.md)）。候选：另立批次把这 28 条改为「`:where()` + 变量 + 基类 fallback」，并把 G1 规则面扩到「尺寸档位修饰符块（无论是否 `:where()`）」 | 中 |
