...
| 候选 | 来源 | 说明 | 优先级 |
|------|------|------|--------|
| 实底前景 token 配对复核 | M3 复核 | 复核其余实底消费点的 `-solid` × `-foreground` 配对，避免跨主题配对冲突 | 中 |
...
| 对比度遗留项盘点 | 亮色 soft 变体 primary 文本 4.37:1 等 4 项；预设品牌色既有例外长期跟踪 | 中 |
| nav/sidebar 链接校验 | `themeConfig.nav/sidebar` 链接不在 `check-links` 覆盖内 | 中 |
| 文档站锚点校验与侧栏不变式 | VitePress slug 不匹配检测 + 侧栏不变量脚本 | 中 |
| 迁移口径一致性守卫 | 入口表 / 组件页节 ↔ §7 一致性机检 | 低 |
| 英文文档同步治理 | parity / freshness 校验与未翻译页回链策略 | 中 |
| a11y 自动化回归 | 引入 axe-core 对关键组件做可访问性断言 | 中 |
| 测试隔离与偶发失败 | Reka + happy-dom 并发时序问题；按「多次出现再处理」跟踪 | 中 |
| 文档完整性守卫的阶段归档误报 | `check-docs-integrity.mjs` 归档后常驻告警 | 低 |
| Review Gate 证据留存 | 评审结论与截图归档到 `artifacts/review-gate/` | 低 |
| 文档站版本化 | 依赖首版发布后的版本基线 | 低 |
| 文档站演示动画遗留项 | keyframes 副本一致性、示例样式不入 stylelint | 低 |
| 文档站示例的外部图片依赖 | `picsum.photos` 外链风险 | 低 |
| 触发器 `unstyled` 遗留收敛 | date-picker / color-picker / split-button 的 `as-child` 绕过 | 低 |
| 代码注释 / 测试名的规划编号守卫 | 机检规则拦截规划编号 | 低 |
| 治理记录索引完整性 | `docs/design/governance/index.md` 需人工维护 | 低 |
| 治理记录的历史规划指针失效 | 归档后指针指向已清空段落 | 低 |
...
| 重复声明守卫 + 同类残留清理 | 8 处 / 4 组件同名属性重复覆盖 | 中 |
| 等价验证产物入库 | 采样脚本 + 冻结基线入库 | 低 |
| 尺寸档位类未 `:where()` 归一化 | 28 条 / 8 组件待用户裁定 | 中 |
| wisdom 蒸馏的机检完备性 | 蒸馏计数对账脚本化 | 低 |
| 文档站观感与展示力 | 待后续评估（用户决策：候选先留在 Backlog） | 中 |
| CHANGELOG 生成器健壮性收口 | 无 remote 降级、语言源自 root | 低 |
| 重复声明（死声明）守卫 + 同类残留清理 | 8 处 / 4 组件同名属性重复覆盖 | 中 |
| 文档站版本化 | 首版发布前无版本基线可切 | 低 |
