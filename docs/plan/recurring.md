...
|------|----------|----------|----------|------|
| 代码复用治理 | 同一模式 ≥3 处且语义一致，且抽取后净收益为正 | 阶段收口前 / 发布前 | 2026-09-19（第 8 轮：门槛复核；第 7 轮：交付「label 优先级全库统一」） | 进行中（暂无待执行批次；条件触发候选见 §2.1） |
| 样式重复收敛 | 同一视觉效果在 ≥3 个组件重复，且已存在或可归纳为语义 token | 阶段收口前 / 发布前 | 2026-09-19（第 8 轮：门槛复核；第 6 轮：交付「禁用态 0.5 档归并」） | 进行中（暂无待执行批次；条件触发候选见 §2.2） |
...
> **触发义务**（[规划规范 §8](../standards/planning.md)）：每个阶段收口前与每次发布前各执行一轮；未留下执行记录视为未执行。第 8 轮已在本次 Phase 7 第二阶段归档前执行并留痕（上表）。
