...
|------|----------|----------|----------|------|
| 代码复用治理 | 同一模式 ≥3 处且语义一致，且抽取后净收益为正 | 阶段收口前 / 发布前 | 2026-09-17（第 5 轮：交付「标签属性转发」；第 6 轮：交付「根可访问控件 label 优先级统一」；第 7 轮：交付「label 优先级全库统一」） | 进行中（暂无待执行批次；条件触发候选见 §2.1） |
...
- **已交付（第 7 轮）**：**label 优先级全库统一**——在 4 组件批次之上做全库审计（逐组件探针核对「组件自带名是否与消费者透传 `aria-label` 落在同一元素」），新增 `_shared/use-label-attrs` 的 `resolveLabelName` 单点定义三级优先级，并把 `progress-bar` / `toast`（视口名）/ `stepper` / `date-picker` / `color-picker` / `slider`（单滑块）/ `split-button` / `calendar`（面板名）/ `file-upload` 收敛到该契约；`paginator` / `progress-spinner` 由内联链重构为同一 helper。审计确认其余 `label` 组件已合规（`popover` 无 `label` prop；`avatar` 用 `alt`，属另一语义）。证据：`docs/design/governance/2026-09-17-label-priority-unification-ui-validation.md`（真实浏览器计算可访问名 51 项）。
