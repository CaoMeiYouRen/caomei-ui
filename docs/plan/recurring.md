...
> **触发义务**（[规划规范 §8](../standards/planning.md)）：每个阶段收口前与每次发布前各执行一轮；未留下执行记录视为未执行。0.1.0 发布前的义务由同日第 8 轮覆盖（Phase 7 第二阶段归档前），第 9 轮为 Phase 5 第二阶段收口前触发并留痕，第 10 轮为 Phase 11 收口前触发并留痕（上表）。
