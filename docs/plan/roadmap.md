...
- Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强：docs 质量护栏 / 站内搜索 / 站点 i18n）与 Phase 6（组件库补全与规范化）已完成并归档；Phase 5 第二阶段（首版发布 / 首个下游接入）待外部前置；Phase 7（下游迁移，momei 优先）、Phase 8（下游兼容性回归）未启动；首版尚未发布。
...
> 状态：Phase 0 ~ Phase 4、Phase 5 第一阶段与 Phase 6 已完成并归档（见 [待办归档](./todo-archive.md)）；当前无已授权启动的阶段，[待办事项](./todo.md) 当前阶段为空；Phase 5 第二阶段（首版发布 / 首个下游接入）待用户外部前置就绪后决策；Phase 7、Phase 8 未启动。
