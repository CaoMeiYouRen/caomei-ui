...
| **Phase 7** | — | 下游迁移（momei 优先，两段式） | 第一阶段（进行中）：迁移就绪——消费路径与 Nuxt 接入（本地 link 先行）、组件 i18n 注入机制、P0 增强、5 个延后组件（见 [Phase 7 第一阶段评估记录](./phase7-evaluation.md)）；第二阶段：momei 迁移闭环 + P1 增强，再推 caomei-auth / rss-impact-next / afdian-linker |
...
>
> **Phase 7 分两段**（2026-09-14 用户授权，评估见 [Phase 7 第一阶段评估记录](./phase7-evaluation.md)）：**第一阶段「迁移就绪」** 先打通消费与接入通道、补齐迁移 P0 高频缺口并前置组件 i18n 注入机制；**第二阶段** 执行 momei 迁移闭环（含 P1 增强）与其他下游接入。排序原则为「先改好再迁移」，按优先级 + 依赖关系推进。消费路径为**本地 link 先行**；首版发布仍归属 Phase 5 第二阶段（待外部 npm 凭据），Phase 7 M1 仅作为并行依赖协调、不重复承载。
...
> 状态：Phase 0 ~ Phase 4、Phase 5 第一阶段与 Phase 6 已完成并归档（见 [待办归档](./todo-archive.md)）；**Phase 7 第一阶段（迁移就绪，momei 优先）已授权启动**（评估见 [Phase 7 第一阶段评估记录](./phase7-evaluation.md)，条目见 [待办事项](./todo.md)）；Phase 5 第二阶段（首版发布 / 首个下游接入）待用户外部前置就绪后决策；Phase 7 第二阶段、Phase 8 未启动。
