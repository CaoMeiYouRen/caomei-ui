...
- Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强：docs 质量护栏 / 站内搜索 / 站点 i18n）、Phase 6（组件库补全与规范化）、Phase 7 第一阶段（迁移就绪，momei 优先）、Phase 9（发布前收口：文档站与代码质量）、Phase 10（国际化与移动端适配）、**Phase 7 第二阶段（库侧迁移就绪与交接计划，2026-09-19）** 与 **Phase 5 第二阶段（首版发布，2026-09-19）已完成并归档**；**Phase 11（组件样式按需化与能力增强，2026-09-20 授权启动）已完成并归档**；**Phase 12（发布就绪、文档对外与一致性收官，2026-09-21 授权启动 / 2026-09-23 完成并归档）已完成并归档**；**Phase 13（组件能力补齐与 dependfix 迁移解阻）：2026-09-23 用户裁定 D1~D9 后登记，2026-09-24 完成并归档**；**Phase 14（质量与一致性收口）：2026-09-25 用户裁定 D1~D10 后登记（登记 = 范围授权，实施未启动）**。Phase 8（下游兼容性回归）未启动（用户 2026-09-23 裁定 D8：等 dependfix 完成迁移后再评估）；首版 0.1.0（2026-09-19）、0.2.0（2026-09-22，含破坏性包形态变更 `styles.css` → `theme.css`）与 **0.3.0（2026-09-24，含 `DataTable` 行分组 / 可折叠分组 / 行展开 / 多列排序与 `TagsInput`，并明确 0.x API 冻结窗口）** 均已发布到 npm（`latest` = 0.3.0），npm 同作者 `0.0.0` 占位保留不 deprecate（用户决策，已发布版本不改）。momei 侧迁移由 momei 项目自行执行、本仓等待反馈；下游接入验证后置为发布后由下游实际迁移反馈驱动。
...
| **Phase 5** | — | 文档站 + 首个下游接入（两段式；第一阶段已完成归档） | 第一阶段：docs 质量护栏、站内搜索、站点 i18n（本仓库可独立闭环）；第二阶段（**2026-09-19 授权启动，同日完成并归档**）：**首版发布（本地手动发布，首版停在 0.x；0.1.0 已发布）**——0.1.0 版本基线与 `v0.1.0` tag、本地手动发布 runbook 与首发执行、发布后校验；**CI 自动发布暂缓**（`release.yml` 发布步骤保持关闭）；**下游接入验证后置**为发布后由下游实际迁移反馈驱动；文档站版本化后置（**2026-09-22 再评估**：多版本托管不做、退回 Backlog，轻量「版本信息与兼容策略」纳入 Phase 12 M2-1），见 [文档与演示站](../design/documentation-site.md) |
...
| **Phase 11** | — | 组件样式按需化与能力增强（**2026-09-20 授权启动，已完成并归档**） | 样式交付按需化（**构建路径 POC 已交付**；`theme.css` 稳定入口与全量入口语义落定含待验项消除 → `unbundle: true` + `css.inject: true` 落地与四处适配：`exports` / resolver / Nuxt 模块 / `check:build` 断言；**已取消逐组件样式入口与依赖闭包批次**，形态依据见 [M1-1 构建路径 POC](../design/governance/2026-09-20-m1-1-build-path-poc.md)）；重量级组件优化与样式治理（scoped 变量声明 / 档位死声明 / 禁用态字面量 / z-index token）；组件能力增强（Select 分组、Tag · Badge、DropdownMenu `model` 扩展、分组按钮可访问语义、AutoComplete 严格选项模式）；质量门与文档守卫（覆盖率门禁改由周期回归与 release 调用、文档结构守卫、README.en-US 与 en-US 响应式修复）。条目见[待办归档](./todo-archive.md)，范围依据见[下一阶段范围评估](../design/governance/2026-09-20-next-stage-scope-evaluation.md) |
| **Phase 12** | — | 发布就绪、文档对外与一致性收官（**2026-09-21 授权启动，2026-09-23 完成并归档**） | 取向为组合 **P / Q / R 混合**（用户决策 2026-09-21），5 条主线 / 20 条原子条目：**M1** 手动发布与版本基线（0.2.0，含 Phase 11 破坏性包形态 `styles.css` → `theme.css`；**不启用 CI 自动发布**）；**M2** 文档对外可用性（版本信息与兼容策略 / 锚点与侧栏不变式 / 英文同步治理 / nav · sidebar 链接校验 / 观感与展示力（组件画廊，条件条目）；**多版本托管经 2026-09-22 再评估后不做**，退回 Backlog）；**M3** 样式一致性收官（28 条尺寸档位 `:where()` 归一化 / `check:design` 规则面扩围 / 重复声明守卫 / 触发器 `unstyled` 收敛 / 计算样式取证装置入库）；**M4** 可访问性自动化回归（axe-core，先立例外清单）；**M5** 治理守卫精选（规划编号 / 治理索引与指针 / 归档误报 / wisdom 机检 / 载体一致性缺口）。条目见[待办归档](./todo-archive.md)，范围依据见[下一阶段范围评估](../design/governance/2026-09-21-next-stage-scope-evaluation.md) |
| **Phase 13** | — | 组件能力补齐与 dependfix 迁移解阻（**2026-09-23 用户裁定 D1~D9 后登记，2026-09-24 完成并归档**） | 下游 dependfix `apps/platform` 迁移已判定「可行（有条件）」，阻塞点为本仓 `DataTable` 三项结构性能力与 1 个缺失组件；本阶段做**库侧补齐**：**M1** `DataTable` 能力补齐（行分组 / 可折叠分组 / 行展开 / 多列排序与降序优先，**API 命名对齐 PrimeVue**）；**M2** `TagsInput` 组件（封装 Reka primitive）；**M3** 迁移交付面（迁移映射与不支持清单收口 / **0.3.0** 版本交付 / 0.x API 冻结窗口声明）；**M4** 治理守卫精选（文档站双花括号插值机检）。4 条主线 / 11 条原子条目。条目见[待办归档](./todo-archive.md)，范围依据见[下一阶段范围评估](../design/governance/2026-09-23-next-stage-scope-evaluation.md) §6 / §11 |
| **Phase 14** | — | 质量与一致性收口（**2026-09-25 用户裁定 D1~D10 后登记；登记 = 范围授权，实施未启动**） | 取向为**「质量与治理收口 + 组件能力与长尾」**（下游迁移护航 / Phase 8 等待相关项目反馈、暂不列入），5 条主线 / **14 条原子条目**：**M1** a11y 例外处置与受检面扩面（`Calendar` 标签落点 / `MultiSelect` 关闭态空 `aria-controls` / `Stepper` 悬空 `aria-describedby`；`Toast` 焦点哨兵例外维持现状；受检面只纳入可稳定驱动者）；**M2** 样式与 token 一致性收口（实底前景配对复核**仅出清单** / 对比度遗留项**仅盘点不改色** / 触发器 `disabled` 透传归一化 / 样式侧旧命名裸类 + `check-design` 解析健壮性）；**M3** 测试稳定性与质量装置（flaky 根因定位并修 + Review Gate 证据留存）；**M4** 治理守卫精选（内置文案台账对账 / 组件总览页成员对账 / 迁移口径一致性）；**M5** 组件能力与长尾判定表（**不纳入实现**）。**不发布新版本**（D10）；字段 shell 样式层共享由[长期任务台账](./recurring.md) 跟踪（不占阶段条目）。范围依据见[下一阶段范围评估](../design/governance/2026-09-25-next-stage-scope-evaluation.md) §10 |

> **规划纪律（2026-09-16 用户决策，规则见 [规划规范 §4](../standards/planning.md)）**：阶段编号只在**授权启动**时分配（取已有最大编号 +1），未经授权不再为将来可能做的工作预留编号、也不提前把未授权范围登记为独立阶段——**未规划过的条目不获得阶段编号**，以保证编号随时间单调递增。表中未启动的 **Phase 5 第二阶段**属已授权阶段内部的分段（沿用既有约定）；**Phase 7 第二阶段已于 2026-09-17 授权启动、2026-09-19 完成归档**；**Phase 5 第二阶段已于 2026-09-19 授权启动**；**Phase 11 已于 2026-09-20 授权启动**；**Phase 12 已于 2026-09-21 授权启动、2026-09-23 完成归档**；**Phase 13 已于 2026-09-23 由用户裁定 D1~D9（范围与取向）后登记（登记 = 范围授权；实施以逐条开工为准）**；**Phase 14 已于 2026-09-25 由用户裁定 D1~D10（范围与取向）后登记（登记 = 范围授权；实施以逐条开工为准）**；**Phase 8** 为规则确立前的历史预留编号；Phase 8 的范围须在启动前单独评估并获得授权（用户 2026-09-23 裁定 D8：维持未启动，等 dependfix 完成迁移后再评估）。
...
> 状态：Phase 0 ~ Phase 4、Phase 5 第一阶段、Phase 6、Phase 7 第一阶段、Phase 9、Phase 10、Phase 7 第二阶段、Phase 5 第二阶段、**Phase 11**、**Phase 12** 与 **Phase 13** 已完成并归档（见 [待办归档](./todo-archive.md)）；**当前进行中阶段：Phase 14（质量与一致性收口）**——2026-09-25 用户裁定 D1~D10 后登记（**登记 = 范围授权，实施未启动**；后续阶段仍须在授权启动时按 [规划规范 §4](../standards/planning.md) 分配编号，未授权范围不预留编号）。**Phase 7 第二阶段（库侧迁移就绪与交接计划）的 momei 侧迁移由 momei 项目执行、本仓等待反馈**；Phase 8 未启动（等待下游完成接入）。
...
已完成阶段记录迁入 [todo-archive.md](./todo-archive.md)：Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件，含预落地的 Switch）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强）、Phase 6（组件库补全与规范化）、Phase 7 第一阶段（迁移就绪，momei 优先）、Phase 9（发布前收口）、Phase 10（国际化与移动端适配）、Phase 7 第二阶段（库侧迁移就绪与交接计划）、Phase 5 第二阶段（首版发布）、**Phase 11（组件样式按需化与能力增强）**、**Phase 12（发布就绪、文档对外与一致性收官）** 与 **Phase 13（组件能力补齐与 dependfix 迁移解阻）** 均已归档。
