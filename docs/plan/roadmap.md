...
- Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强：docs 质量护栏 / 站内搜索 / 站点 i18n）、Phase 6（组件库补全与规范化）、Phase 7 第一阶段（迁移就绪，momei 优先）、Phase 9（发布前收口：文档站与代码质量）、Phase 10（国际化与移动端适配）、**Phase 7 第二阶段（库侧迁移就绪与交接计划，2026-09-19）** 与 **Phase 5 第二阶段（首版发布，2026-09-19）已完成并归档**；**Phase 11（组件样式按需化与能力增强，2026-09-20 授权启动）已完成并归档**；**Phase 12（发布就绪、文档对外与一致性收官，2026-09-21 授权启动）为当前进行中阶段**。Phase 8（下游兼容性回归）未启动（用户 2026-09-21 裁定：等待下游完成接入）；首版 0.1.0（2026-09-19）与当前版本 **0.2.0（2026-09-22，含破坏性包形态变更 `styles.css` → `theme.css`）** 均已发布到 npm（`latest` = 0.2.0），npm 同作者 `0.0.0` 占位保留不 deprecate（用户决策，已发布版本不改）。momei 侧迁移由 momei 项目自行执行、本仓等待反馈；下游接入验证后置为发布后由下游实际迁移反馈驱动。
...
| **Phase 12** | — | 发布就绪、文档对外与一致性收官（**2026-09-21 授权启动，进行中**） | 取向为组合 **P / Q / R 混合**（用户决策 2026-09-21），5 条主线 / 20 条原子条目：**M1** 手动发布与版本基线（0.2.0，含 Phase 11 破坏性包形态 `styles.css` → `theme.css`；**不启用 CI 自动发布**）；**M2** 文档对外可用性（M2-1 版本信息与兼容策略 / M2-2 锚点与侧栏不变式 / M2-3 英文同步治理 / M2-4 nav · sidebar 链接校验；M2-5 观感与展示力为条件条目；**多版本托管经 2026-09-22 再评估后不做**，退回 Backlog）；**M3** 样式一致性收官（28 条尺寸档位 `:where()` 归一化 / `check:design` 规则面扩围 / 重复声明守卫 / 触发器 `unstyled` 收敛 / 计算样式取证装置入库）；**M4** 可访问性自动化回归（axe-core，先立例外清单）；**M5** 治理守卫精选（规划编号 / 治理索引与指针 / 归档误报 / wisdom 机检 / 载体一致性缺口）。条目见[待办事项](./todo.md)，范围依据见[下一阶段范围评估](../design/governance/2026-09-21-next-stage-scope-evaluation.md) |

> **规划纪律（2026-09-16 用户决策，规则见 [规划规范 §4](../standards/planning.md)）**：阶段编号只在**授权启动**时分配（取已有最大编号 +1），未经授权不再为将来可能做的工作预留编号、也不提前把未授权范围登记为独立阶段——**未规划过的条目不获得阶段编号**，以保证编号随时间单调递增。表中未启动的 **Phase 5 第二阶段**属已授权阶段内部的分段（沿用既有约定）；**Phase 7 第二阶段已于 2026-09-17 授权启动、2026-09-19 完成归档**；**Phase 5 第二阶段已于 2026-09-19 授权启动**；**Phase 11 已于 2026-09-20 授权启动**；**Phase 12 已于 2026-09-21 授权启动**；**Phase 8** 为规则确立前的历史预留编号；Phase 8 的范围须在启动前单独评估并获得授权（用户 2026-09-21 裁定维持未启动、等待下游完成接入）。
...
> 状态：Phase 0 ~ Phase 4、Phase 5 第一阶段、Phase 6、Phase 7 第一阶段、Phase 9、Phase 10、Phase 7 第二阶段、Phase 5 第二阶段与 **Phase 11** 已完成并归档（见 [待办归档](./todo-archive.md)）；**当前进行中阶段：Phase 12（发布就绪、文档对外与一致性收官，2026-09-21 授权启动）**。**Phase 7 第二阶段（库侧迁移就绪与交接计划）的 momei 侧迁移由 momei 项目执行、本仓等待反馈**；Phase 8 未启动（等待下游完成接入）。
