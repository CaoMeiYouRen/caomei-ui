...
- Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强：docs 质量护栏 / 站内搜索 / 站点 i18n）、Phase 6（组件库补全与规范化）与 Phase 7 第一阶段（迁移就绪，momei 优先）已完成并归档；**Phase 9（发布前收口：文档站与代码质量）已于 2026-09-16 获授权启动**；Phase 5 第二阶段（首版发布 / 首个下游接入）待外部前置；Phase 7 第二阶段（momei 迁移闭环 + P1 增强）按用户决策排在本阶段之后；Phase 8（下游兼容性回归）未启动；首版尚未发布。
...
| **Phase 7** | — | 下游迁移（momei 优先，两段式） | 第一阶段（**已完成并归档**）：迁移就绪——消费路径与 Nuxt 接入（本地 link 先行）、组件 i18n 注入机制、P0 增强、5 个延后组件（见 [待办归档](./todo-archive.md)）；第二阶段（未启动）：momei 迁移闭环 + P1 增强，再推 caomei-auth / rss-impact-next / afdian-linker（按用户决策排在 Phase 9 之后） |
| **Phase 8** | — | 下游兼容性回归机制（稳定后启用） | 已接入下游清单 + 跨仓库 CI 触发；仅在稳定使用且组件库改动时启用 |
| **Phase 9** | — | 发布前收口：文档站与代码质量（**进行中**） | 文档站信息架构重构（6 分组 + 能力说明归位 + zh 总览页）、默认主题主色改蓝、演示动画 opt-in、公共逻辑抽取、ESLint 严格化与导出类型（条目见 [待办事项](./todo.md)，范围依据见 [下一阶段评估记录](../design/governance/2026-09-16-pre-release-stage-evaluation.md)） |
...
> 状态：Phase 0 ~ Phase 4、Phase 5 第一阶段、Phase 6 与 Phase 7 第一阶段（迁移就绪，momei 优先）已完成并归档（见 [待办归档](./todo-archive.md)）；当前进行中为 **Phase 9（发布前收口：文档站与代码质量）**，条目见 [待办事项](./todo.md)、范围依据见 [下一阶段评估记录](../design/governance/2026-09-16-pre-release-stage-evaluation.md)；Phase 5 第二阶段（首版发布 / 首个下游接入）待用户外部前置就绪后决策；Phase 7 第二阶段（momei 迁移闭环 + P1 增强，排在 Phase 9 之后）、Phase 8 未启动。
...
已完成阶段记录迁入 [todo-archive.md](./todo-archive.md)：Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件，含预落地的 Switch）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强）、Phase 6（组件库补全与规范化）与 Phase 7 第一阶段（迁移就绪，momei 优先）均已归档。
