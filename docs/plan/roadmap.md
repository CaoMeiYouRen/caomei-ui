...
| **Phase 11** | — | 组件样式按需化与能力增强（**2026-09-20 授权启动，进行中**） | 样式交付按需化（**构建路径 POC 已交付**；`theme.css` 稳定入口与全量入口语义落定含待验项消除 → `unbundle: true` + `css.inject: true` 落地与四处适配：`exports` / resolver / Nuxt 模块 / `check:build` 断言；**已取消逐组件样式入口与依赖闭包批次**，形态依据见 [M1-1 构建路径 POC](../design/governance/2026-09-20-m1-1-build-path-poc.md)）；重量级组件优化与样式治理（scoped 变量声明 / 档位死声明 / 禁用态字面量 / z-index token）；组件能力增强（Select 分组、Tag · Badge、DropdownMenu `model` 扩展、分组按钮可访问语义）；质量门与文档守卫（覆盖率门禁改由周期回归与 release 调用、文档结构守卫、README.en-US 与 en-US 响应式修复）。条目见[待办事项](./todo.md)，范围依据见[下一阶段范围评估](../design/governance/2026-09-20-next-stage-scope-evaluation.md) |
