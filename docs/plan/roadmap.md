...
| **Phase 12** | — | 发布就绪、文档对外与一致性收官（**2026-09-21 授权启动，进行中**） | 取向为组合 **P / Q / R 混合**（用户决策 2026-09-21），5 条主线 / 20 条原子条目：**M1** 手动发布与版本基线（0.2.0，含 Phase 11 破坏性包形态 `styles.css` → `theme.css`；**不启用 CI 自动发布**）；**M2** 文档站版本化与对外可用性（版本化 / 锚点与侧栏不变式 / 英文同步治理 / nav · sidebar 链接校验；观感与展示力为条件条目）；**M3** 样式一致性收官（28 条尺寸档位 `:where()` 归一化 / `check:design` 规则面扩围 / 重复声明守卫 / 触发器 `unstyled` 收敛 / 计算样式取证装置入库）；**M4** 可访问性自动化回归（axe-core，先立例外清单）；**M5** 治理守卫精选（规划编号 / 治理索引与指针 / 归档误报 / wisdom 机检 / 载体一致性缺口）。条目见[待办事项](./todo.md)，范围依据见[下一阶段范围评估](../design/governance/2026-09-21-next-stage-scope-evaluation.md) |
