...
- Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强：docs 质量护栏 / 站内搜索 / 站点 i18n）、Phase 6（组件库补全与规范化）、Phase 7 第一阶段（迁移就绪，momei 优先）、Phase 9（发布前收口：文档站与代码质量）、Phase 10（国际化与移动端适配）、**Phase 7 第二阶段（库侧迁移就绪与交接计划，2026-09-19）** 与 **Phase 5 第二阶段（首版发布，2026-09-19）已完成并归档**；**Phase 11（组件样式按需化与能力增强，2026-09-20 授权启动）已完成并归档**；**Phase 12（发布就绪、文档对外与一致性收官，2026-09-21 授权启动 / 2026-09-23 完成并归档）已完成并归档**；**Phase 13（组件能力补齐与 dependfix 迁移解阻）：2026-09-23 用户裁定 D1~D9 后登记，2026-09-24 完成并归档**。Phase 8（下游兼容性回归）未启动（用户 2026-09-23 裁定 D8：等 dependfix 完成迁移后再评估）；首版 0.1.0（2026-09-19）、0.2.0（2026-09-22，含破坏性包形态变更 `styles.css` → `theme.css`）与 **0.3.0（2026-09-24，含 `DataTable` 行分组 / 可折叠分组 / 行展开 / 多列排序与 `TagsInput`，并明确 0.x API 冻结窗口）** 均已发布到 npm（`latest` = 0.3.0），npm 同作者 `0.0.0` 占位保留不 deprecate（用户决策，已发布版本不改）。momei 侧迁移由 momei 项目自行执行、本仓等待反馈；下游接入验证后置为发布后由下游实际迁移反馈驱动。
...
> **规划纪律（2026-09-16 用户决策，规则见 [规划规范 §4](../standards/planning.md)）**：阶段编号只在**授权启动**时分配（取已有最大编号 +1），未经授权不再为将来可能做的工作预留编号、也不提前把未授权范围登记为独立阶段——**未规划过的条目不获得阶段编号**，以保证编号随时间单调递增。表中未启动的 **Phase 5 第二阶段**属已授权阶段内部的分段（沿用既有约定）；**Phase 7 第二阶段已于 2026-09-17 授权启动、2026-09-19 完成归档**；**Phase 5 第二阶段已于 2026-09-19 授权启动**；**Phase 11 已于 2026-09-20 授权启动**；**Phase 12 已于 2026-09-21 授权启动、2026-09-23 完成归档**；**Phase 13 已于 2026-09-23 由用户裁定 D1~D9（范围与取向）后登记（登记 = 范围授权；实施以逐条开工为准）**；**Phase 8** 为规则确立前的历史预留编号；Phase 8 的范围须在启动前单独评估并获得授权（用户 2026-09-23 裁定 D8：维持未启动，等 dependfix 完成迁移后再评估）。
...
> 状态：Phase 0 ~ Phase 4、Phase 5 第一阶段、Phase 6、Phase 7 第一阶段、Phase 9、Phase 10、Phase 7 第二阶段、Phase 5 第二阶段、**Phase 11**、**Phase 12** 与 **Phase 13** 已完成并归档（见 [待办归档](./todo-archive.md)）；**当前无进行中阶段**（下一阶段须在授权启动时按 [规划规范 §4](../standards/planning.md) 分配编号，未授权范围不预留编号）。**Phase 7 第二阶段（库侧迁移就绪与交接计划）的 momei 侧迁移由 momei 项目执行、本仓等待反馈**；Phase 8 未启动（等待下游完成接入）。
