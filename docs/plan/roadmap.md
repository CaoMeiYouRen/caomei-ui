...
- Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强：docs 质量护栏 / 站内搜索 / 站点 i18n）、Phase 6（组件库补全与规范化）、Phase 7 第一阶段（迁移就绪，momei 优先）、Phase 9（发布前收口：文档站与代码质量）、Phase 10（国际化与移动端适配）、**Phase 7 第二阶段（库侧迁移就绪与交接计划，2026-09-19）** 与 **Phase 5 第二阶段（首版发布，2026-09-19）已完成并归档**；**Phase 11（组件样式按需化与能力增强，2026-09-20 授权启动）已完成并归档**；**Phase 12（发布就绪、文档对外与一致性收官，2026-09-21 授权启动 / 2026-09-23 完成并归档）已完成并归档**；**当前阶段为 Phase 13（组件能力补齐与 dependfix 迁移解阻）：2026-09-23 用户裁定 D1~D9 后登记（登记 = 范围授权，实施未启动）**。Phase 8（下游兼容性回归）未启动（用户 2026-09-23 裁定 D8：等 dependfix 完成迁移后再评估）；首版 0.1.0（2026-09-19）与当前版本 **0.2.0（2026-09-22，含破坏性包形态变更 `styles.css` → `theme.css`）** 均已发布到 npm（`latest` = 0.2.0），npm 同作者 `0.0.0` 占位保留不 deprecate（用户决策，已发布版本不改）。momei 侧迁移由 momei 项目自行执行、本仓等待反馈；下游接入验证后置为发布后由下游实际迁移反馈驱动。
