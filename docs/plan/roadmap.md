...
- Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强：docs 质量护栏 / 站内搜索 / 站点 i18n）、Phase 6（组件库补全与规范化）、Phase 7 第一阶段（迁移就绪，momei 优先）、Phase 9（发布前收口：文档站与代码质量）、Phase 10（国际化与移动端适配）、**Phase 7 第二阶段（库侧迁移就绪与交接计划，2026-09-19）** 与 **Phase 5 第二阶段（首版发布，2026-09-19）已完成并归档**；**Phase 11（组件样式按需化与能力增强，2026-09-20 授权启动）已完成并归档**；**Phase 12（发布就绪、文档对外与一致性收官，2026-09-21 授权启动 / 2026-09-23 完成并归档）已完成并归档**；**当前阶段为 Phase 13（组件能力补齐与 dependfix 迁移解阻）：2026-09-23 用户裁定 D1~D9 后登记；条目已全部交付，阶段收口与归档待执行**。Phase 8（下游兼容性回归）未启动（用户 2026-09-23 裁定 D8：等 dependfix 完成迁移后再评估）；首版 0.1.0（2026-09-19）、0.2.0（2026-09-22，含破坏性包形态变更 `styles.css` → `theme.css`）与 **0.3.0（2026-09-24，含 `DataTable` 行分组 / 可折叠分组 / 行展开 / 多列排序与 `TagsInput`，并明确 0.x API 冻结窗口）** 均已发布到 npm（`latest` = 0.3.0），npm 同作者 `0.0.0` 占位保留不 deprecate（用户决策，已发布版本不改）。momei 侧迁移由 momei 项目自行执行、本仓等待反馈；下游接入验证后置为发布后由下游实际迁移反馈驱动。
...
| **Phase 13** | — | 组件能力补齐与 dependfix 迁移解阻（**2026-09-23 用户裁定 D1~D9 并登记**） | 下游 dependfix `apps/platform` 迁移已判定「可行（有条件）」，阻塞点为本仓 `DataTable` 三项结构性能力与 1 个缺失组件；本阶段做**库侧补齐**：**M1** `DataTable` 能力补齐（行分组 / 可折叠分组 / 行展开 / 多列排序与降序优先，**API 命名对齐 PrimeVue**）；**M2** `TagsInput` 组件（封装 Reka primitive）；**M3** 迁移交付面（迁移映射与不支持清单收口 / **0.3.0** 版本交付 / 0.x API 冻结窗口声明）；**M4** 治理守卫精选（文档站双花括号插值机检）。4 条主线 / 11 条原子条目。条目见[待办事项](./todo.md)，范围依据见[下一阶段范围评估](../design/governance/2026-09-23-next-stage-scope-evaluation.md) §6 / §11 |
...
> 状态：Phase 0 ~ Phase 4、Phase 5 第一阶段、Phase 6、Phase 7 第一阶段、Phase 9、Phase 10、Phase 7 第二阶段、Phase 5 第二阶段、**Phase 11** 与 **Phase 12** 已完成并归档（见 [待办归档](./todo-archive.md)）；**当前阶段：Phase 13（组件能力补齐与 dependfix 迁移解阻，2026-09-23 用户裁定 D1~D9 后登记；登记 = 范围授权，实施未启动）**。**Phase 7 第二阶段（库侧迁移就绪与交接计划）的 momei 侧迁移由 momei 项目执行、本仓等待反馈**；Phase 8 未启动（等待下游完成接入）。
...
已完成阶段记录迁入 [todo-archive.md](./todo-archive.md)：Phase 0（立项与 POC）、Phase 1（Tier 0 组件）、Phase 2（Tier 1 组件，含预落地的 Switch）、Phase 3（Tier 2 组件）、Phase 4（Tier 3 稳定批组件）、Phase 5 第一阶段（文档站增强）、Phase 6（组件库补全与规范化）、Phase 7 第一阶段（迁移就绪，momei 优先）、Phase 9（发布前收口）、Phase 10（国际化与移动端适配）、Phase 7 第二阶段（库侧迁移就绪与交接计划）、Phase 5 第二阶段（首版发布）、**Phase 11（组件样式按需化与能力增强）** 与 **Phase 12（发布就绪、文档对外与一致性收官）** 均已归档。
