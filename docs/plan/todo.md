...
| 编号 | 条目 | 范围 | 最小验收标准 | 依赖 |
| :-: | --- | --- | --- | :-: |
| M2-1 | `--p-*` → `--caomei-*` token 对照表 | 建 `docs/design/governance/2026-09-17-momei-migration-assets.md` §1；覆盖 Top 前缀（surface 492 / text 381 / primary 252 / content 126）与高频 token，逐条给出本库对应 token 或「无对应 + 处理方式」 | 对照表非空且与评估记录 §2.3 计数口径一致；Top 前缀逐条有对应或处理方式；取证命令与快照日期随表记录 | M1-1 |
| M2-2 | `pi pi-*` → lucide 图标映射表 | 同文档 §2；128 个唯一图标逐行给出 `@lucide/vue` 落点或「无对应 + 处理方式」 | 映射行数 = 128；逐行有落点或处理方式；计数与评估记录 §2.3 一致 | M1-1 |
| M2-3 | 双库并存隔离策略 + 包体监控口径 | 同文档 §3；并存期路由 / 页面白名单的载体与切换粒度；包体监控记录项与命令 | 策略含白名单载体与切换粒度；包体口径含记录项与命令，且与[交接计划 §9](../design/governance/2026-09-17-momei-migration-handover-plan.md) 一致 | M1-1 |

状态：M2-1 ~ M2-3 为**已登记（范围授权）**，尚未交付。
