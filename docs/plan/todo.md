...
**B2 已交付（2026-09-18）**：M5-3（ConfirmDialog `icon`）+ M5-4（Popover 命令式收敛为声明式迁移写法）。要点：① `useConfirm` 请求项新增 `icon`（`@lucide/vue` 组件，不接受字符串类名），缺省按 `tone` 回退内建图标（`neutral` → `Info`、`danger` → `TriangleAlert`，属有意增强），store 的 request 改 `shallowRef` 以消除传入组件时的深代理告警；② 经 D 阶段取证**不新增命令式入口**——原触发按钮本身即 `CaomeiPopoverTrigger`（锚点与 `toggle(event)` 一致），并新增 `CaomeiPopoverTrigger` 的 `unstyled` 以支持 `as-child` 复用自定义按钮；momei 6 处 / 5 文件中 4 处可机械等价、2 处跨组件（voice overlay 的 `defineExpose` + `show(event)`）需结构改写，落点已在 §7 与组件页登记。**Review Gate R1 Pass**（0 blocker；3 warning / 4 suggest 全部在 F 前处置：视觉副作用以「保留既有 12px 间距零回归」消除、补「无 reactive object 告警」守卫、§7 与组件页补跨组件落点、`__text` 更名 `__heading` 对齐 Dialog、Backlog 补记）。**V 阶段**：文档站 light / dark × 三态（默认 `Info` / danger `TriangleAlert` / 自定义 `Rocket`）实测图标类名、`aria-hidden`、计算色与 16px 尺寸、标题描述间距 12px、几何断言与 0 console error，OCR 确认暗色文案渲染，记录见 [M5 B2 浏览器验证](../design/governance/2026-09-18-m5-b2-confirm-icon-popover-ui-validation.md)。**质量门**：`pnpm verify` exit 0（test 71 文件 1295 例）。**B2 粒度与拆分**：按[AI 协作规范 §3.2.1](../standards/ai-collaboration.md) 分 6 个提交——① §7 登记 1 文件（58cf0d7）；② M5-3 代码 + 单测 3 文件（f334d86）；③ M5-3 文档与示例 6 文件（71ff417）；④ M5-4 代码 + 单测 3 文件（c66e398）；⑤ M5-4 文档与示例 7 文件（cbb1219）；⑥ 验证记录与规划登记 2 文件。全部 ≤ 10 文件；M5-3 与 M5-4 无同文件并发改，逐条独立提交。
