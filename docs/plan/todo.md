...
- **进度（2026-09-23）**：**M1-1 行分组（subheader）已交付**（12 文件 / +768 −42，拆两次提交；`pnpm test` 84 文件 / 1691 tests、`capture:styles` 239 项 0 差异、`docs:check` 9 段链全绿、`@ui-validator` 复验 8 项全通过、`@code-reviewer` Review Gate 第 1 轮 **Pass**（0 blocker）；交付与验证记录见[治理记录](../design/governance/2026-09-23-m1-1-row-grouping.md)）。M1-2 ~ M1-4、M2、M3、M4 **未启动**。
