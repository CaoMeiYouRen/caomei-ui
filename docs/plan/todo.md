...
- 条目：滚动追加（当前 3 条：M6-1 / M6-2 / M6-3）；触发时机为等待 momei 侧反馈期间；追加规则为「先取证 → 达门槛 → 登记为原子条目 → 按本表自上而下交付」。
