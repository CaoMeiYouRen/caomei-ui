...
- **进度（2026-09-23）**：**M1-1 行分组（subheader）已交付**（12 文件 / +768 −42，拆两次提交；`pnpm test` 84 文件 / 1691 tests、`capture:styles` 239 项 0 差异、`docs:check` 9 段链全绿、`@ui-validator` 复验 8 项全通过、`@code-reviewer` Review Gate 第 1 轮 **Pass**（0 blocker）；交付与验证记录见[治理记录](../design/governance/2026-09-23-m1-1-row-grouping.md)）。**M1-2 可折叠分组已交付**（22 文件 / +664 −16，拆三次提交；`pnpm test` 84 文件 / 1703 tests、`check:locale-keys` 5 语种各 73 条、`capture:styles` 239 项 0 差异、`@ui-validator` 9 项全通过；Review Gate 第 1 轮 **Reject**（1 blocker：内置文案台账未同步新键，无门禁覆盖）→ B1 与 6 条 suggest 同批修正 → 第 2 轮（只审修复点）**Pass**（0 blocker）→ 记录见[治理记录](../design/governance/2026-09-23-m1-2-expandable-row-groups.md)）。**M1-3 行展开已交付**（规模见[治理记录](../design/governance/2026-09-23-m1-3-row-expansion.md) §6，拆三次提交；`pnpm test` 85 文件 / 1718 tests、`check:locale-keys` 5 语种各 75 条、`capture:styles` 239 项 0 差异、`@ui-validator` 两轮全通过；Review Gate 第 1 轮 **Pass**（0 blocker / 1 warning / 5 suggest），W1 / S1 / S2 / S4 已同批修正并刷新 V 证据、S3 / S5 未采纳并登记理由 → 记录见[治理记录](../design/governance/2026-09-23-m1-3-row-expansion.md)）。**M1-4 多列排序与降序优先已交付**（规模见[治理记录](../design/governance/2026-09-23-m1-4-multi-sort.md) §6，拆两次提交；`pnpm test` 86 文件 / 1737 tests、`capture:styles` 239 项 0 差异、`@ui-validator` 9 项全通过；Review Gate 第 1 轮 **Pass**（0 blocker / 1 warning / 3 suggest），RG-W01 / S02 / S03 已同批修正、S01 保留并声明理由）。**M1 主线四条原子条目已全部交付**；**M2-1 `TagsInput` 实现与接线 / M2-2 组件页与示例已交付**（规模见[治理记录](../design/governance/2026-09-23-m2-tags-input.md) §7，拆两次提交；`pnpm test` 87 文件 / 1767 tests、`check:locale-keys` 26 命名空间 / 76 条 × 5、`docs:check` 9 段链全绿（parity 59 对）、`capture:styles` 239 项 0 差异、`@ui-validator` 真机 15 项全通过；Review Gate：M2-1 **R1 Reject（1 blocker：UI 组件缺浏览器证据）→ 建立验证载体并完成真机验证 → R2 Pass**、M2-2 **R1 Pass**，M2-2 的 W1/W2/S1/S2/S3 已同批修正；**M2-1 的 V 证据与 RG 结论见该记录 §6.2 / §6.3 / §9**）。**M2-3 登记面接入已交付**（6 文件 / +17 −2，单批提交；`docs:check` 9 段链全绿（侧栏 6 组 / **47** 条目、画廊 **13** 项）、`pnpm verify` exit 0（87 文件 / 1767 tests）、构建产物 SSG 静态断言 18/18、`@ui-validator` dev 真机 11 项全通过（0 问题 / 1 观察项）、Review Gate R1 **Pass**（0 blocker / 1 warning / 3 suggest，W1 / S1 已同批修正）；交付与验证记录见[治理记录](../design/governance/2026-09-23-m2-tags-input.md) §10~§13）。**M2 三条原子条目全部交付**；**M3-1 迁移映射与不支持清单收口已交付**（2 文件 / +4 −1，单批提交；迁移指南入口表与 §11 / §7 全量对齐 **47/47/47**、补 3 行通用陷阱覆盖 `DataTable` 三项能力与 `TagsInput`；`docs:check` 9 段链全绿、`lint:md` 通过、`docs:build` exit 0；V 阶段显式跳过（纯 markdown 内容）；Review Gate R1 **Pass**（0 blocker / 1 warning / 3 suggest，RG-S02 / RG-S03 已同批修正）；记录见[治理记录](../design/governance/2026-09-23-m3-migration-delivery.md)）。**M3-2 0.3.0 版本交付已交付**（版本基线 `92264fc` / CHANGELOG `75c5610` / annotated tag `v0.3.0` → `92264fc` / 用户本地 `npm publish`；本批补**发布后校验**——registry tarball shasum 与发布日志逐字符一致（350 文件）、含 `dist/styles/index.css`（5,880 B）不含旧单体、四项子路径冒烟（根 89 导出含 `CaomeiTagsInput`）+ Vite 消费方构建通过、`npm view` = 0.3.0、`pnpm verify` 复跑 exit 0；2 项偏差已登记）；**M3-3 0.x 冻结窗口声明已交付**（中英版本策略页新增「0.x API 冻结窗口」节，对齐 dependfix §12 上收条件 3）；发布后文档同步（README 中英版本 / roadmap 版本句与 Phase 13 状态 / 条目计数 10 → 11 订正）；Review Gate R1 **Reject**（2 blocker / 3 warning / 3 suggest）→ 8 条 finding 修正 → R2 复审 **Pass**（0 blocker）；记录见[治理记录](../design/governance/2026-09-24-phase13-m3-2-m3-3-release-and-freeze.md)。**M4-1 双花括号插值机检守卫已交付**（新增 `pnpm docs:check:interpolation` + 26 tests，`docs:check` 由 9 段扩为 **10 段**；仓库级负向对照灵敏（围栏外 / 行内命中、围栏内不报）、全库零误报（受检 211 md）；`pnpm test` 88 文件 / 1793 tests、`docs:check` 10 段链全绿、`governance:check` / `docs:build` exit 0；Review Gate R1 **Pass**（0 blocker / 2 warning / 4 suggest）→ 6 条 finding 修正 → R2 复审 **Reject**（1 blocker：§2 计数为修复前快照、不可复算）→ 计数刷新 → R3 复审 **Pass**（0 blocker）；规模 4 文件 / +540 −1（唯一口径见[治理记录](../design/governance/2026-09-24-m4-1-interpolation-guard.md) §2））。**M4 主线唯一原子条目已交付**。**Phase 13 全部 11 条原子条目交付完毕**，阶段收口与归档待执行。
