...
>
> **用户决策（2026-09-16，M4 范围调整）**：公共逻辑抽取升级为[长期任务](./recurring.md)（不占阶段编号）；M4 本轮先落治理规范、再执行首轮抽取，首轮批次为标签属性转发 / ARIA 布尔假值 / attrs 透传收敛；原列 locale 文本解析与选项列表渲染移出本轮转入长期任务台账（移出理由：净收益待评估 / DOM 一致性待比对），阴影 / 遮罩 token 迁入长期任务「样式重复收敛」（移出理由：需先归纳阴影 token 档位）。
...
- 条目：
  ...
  - [x] 能力说明归位方案 B（三页迁移 + en 镜像 + 引用点与 `check-i18n-routing` 用例同步）
  - [x] zh 组件总览页（分组导航 + 一句话说明，含侧栏总览入口，与 en 侧 `Overview` 对称）
...
- 执行范围：先落治理规范（长期任务机制 + 公共逻辑抽取准则），再执行首轮抽取——标签属性转发（12 处：整体透传 10 + `controlAttrs` 基座 2）、表单控件公共 props 契约（首批 7 文件）、表单控件聚焦控制（4 文件）；经复评，原列的 ARIA 布尔假值归一、attrs 透传收敛、locale 文本解析、选项列表渲染均未达门槛，登记入 [长期任务台账](./recurring.md)。
- 非目标：为抽取而重构；数值钳位（2 份）与焦点模式（4 + 2 + 2）不单列。
- 最小验收标准：每批独立提交且全量测试通过；组件对外行为与 DOM 结构不变（既有单测 + 定向浏览器抽验）；`check:design` 通过；长期任务与抽取准则接入规划体系（[规划规范 §8](../standards/planning.md)、[开发规范](../standards/development.md)「公共逻辑抽取与复用」）。
- 条目：
  - [x] 治理规范落地（[长期任务台账](./recurring.md) + 规划规范长期任务制度 + 开发规范抽取准则）
  - [x] 标签属性转发抽取（12 处）
  - [x] 表单控件公共 props 契约抽取（首批 7 文件）
  - [x] 表单控件聚焦控制抽取（4 文件）

> 计数口径与快照日期见 [长期任务台账](./recurring.md) §2.1 的「取证口径」块（2026-09-16 快照）；[评估记录](../design/governance/2026-09-16-pre-release-stage-evaluation.md) §3 保留的是阶段启动时的原始范围口径，不代表本条目现行范围。
...
- 条目：
  ...
  - [x] 切换 `vue/strict` 并固化门禁（verify 全绿 + 类型产物冒烟）
