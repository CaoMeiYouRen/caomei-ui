...
状态（2026-09-18）：M4-1 / M4-2 / M4-3 / M4-6 / M4-7 **已交付**；**M4-4 / M4-5 待用户裁定 API 形态**——momei 实际用量为「单个 `Checkbox` + 数组 `v-model`」（20 处 / 13 文件，无 `<CheckboxGroup>` 标签），与本文登记的「CheckboxGroup 组件 + 全选 / 半选语义」存在两种解读，属公开 API 设计分歧，已提交用户决策，未擅自实现。交付形态：M4-1 `#option` 插槽（收原始选项对象与选中态）+ `showClear`（有选中项且未禁用时显示，清除后模型置为空数组、焦点交回输入框；清除按钮为字段内常规 flex 成员，与 PrimeVue 的绝对定位形态属有意差异）；M4-2 `rowsPerPageOptions` + `update:itemsPerPage`（页码按**保留首行偏移**语义重推导，对齐 PrimeVue 的 `first` 语义；`disabled` 一并传递；`template` / `CurrentPageReport` 经一方源码与用量取证后登记为**未实现**）；M4-3 `badge` / `badgeTone`（默认 `neutral`，对应 PrimeVue `badge-severity` 默认的 `secondary`）；M4-6 `change`（仅用户交互触发，载荷由 PrimeVue 的原生事件改为布尔值，属有意差异）；M4-7 `onLabel` / `offLabel`（需同时提供才渲染，PrimeVue 内建 `Yes` / `No` 默认值属有意差异）。**Review Gate 两轮**：第 1 轮 Reject（blocker ① Paginator「重置到第 1 页（对齐 PrimeVue）」与 PrimeVue 的偏移保持语义不符且用例无判别力；② ToggleButton「需同时提供才生效（对齐 PrimeVue）」未反映 PrimeVue 的 `Yes` / `No` 默认值；③ 含可见 UI 改动但缺 V 阶段 → 修复后第 2 轮 **Pass**（0 blocker / 0 新增 warning）。**V 阶段**：`@ui-validator` **119 / 119 项通过、失败 0**、观察项 3（均不计缺陷；其中 2 项已随批补文档），记录 [M4 B1 浏览器验证](../design/governance/2026-09-18-m4-b1-data-form-ui-validation.md)。**质量门**：`pnpm verify` exit 0（全量 test 69 文件 1232 例 / build / check:build / check:nuxt / docs:build / docs:check:i18n-routing / governance:check）；期间两次全量运行各命中 1 例既有 flaky（`CaomeiMultiSelect` / `CaomeiDatePicker` 各一例，与本批无关，隔离复跑与后续全量均通过）。提交 29044c1 / 45fa047 / f14d49b / b13a465 / ee989c3 / d3ddf84 / b0776fb（均未推送）。规模 43 文件 / +653（按 locale → 各组件 → 设计登记拆 7 个提交，单条目均未超粒度阈值）。
...
- 执行范围：① 组件缺口复盘（B1 执行中新发现 + [Backlog §1.1 / §1.2](./backlog.md) 候选重新取证）；② 按[长期任务台账](./recurring.md)门槛或 [规划规范 §8](../standards/planning.md) 判定后交付的组件优化批次；③ 文档优化（限当前 Backlog 已有范围，不新扩）；④ 用户缺陷报告（附带截图与页面定位）触发的组件 / 文档缺陷修复——授权锚点为用户报告本身（用户直接指示优先于「评估 → backlog」默认路径），不新增 [规划规范 §3.5](../standards/planning.md) 的插队例外类别；④ 类条目以报告 + 浏览器实测值为取证，不要求 Backlog 出处。
- 非目标：不新扩 Backlog 范围；不为等待期预留条目；不纳入未达门槛的候选；不触碰 momei 文件。
- 最小验收标准：① 复盘结论落可提交位置，每项含取证命令与快照日期；② 交付批次满足对应门槛且逐轮留痕；③ 文档优化条目有明确 Backlog 出处（④ 类条目以用户报告 + 浏览器实测值为取证，不要求 Backlog 出处）。
- 条目：滚动追加（当前 5 条：M6-1 / M6-2 / M6-3 / M6-4 / M6-5）；触发时机为等待 momei 侧反馈期间；追加规则为「先取证 → 达门槛 → 登记为原子条目 → 按本表自上而下交付」。
