...
- 条目：
  ...
  - [x] DataView（`layout` grid / list + 插槽）——说明：自建（`layout` 插槽分发 + 状态区域），Reka 无对应组件。`layout` 默认 `list` 对齐 PrimeVue，`#list` / `#grid` 插槽收到 `{ items }`，另提供 `#header` / `#footer` / `#empty`（收到 `layout`）；只渲染与当前 `layout` 匹配的插槽，不回退。`loading` / `loadingText` 为本库新增——PrimeVue v4 `DataView` 无 `loading` prop（官方建议以 Skeleton 表达加载态，下游传入的 `:loading` 目前不生效）；空态文案走内建 locale `dataView.empty`；`grid` 模式只切换根修饰类与插槽，网格列定义由使用方内容层承担（与 PrimeVue 一致）。**未实现（下游零用量）**：分页（`paginator` / `rows` / `first` / `totalRecords` 等）、排序（`sortField` / `sortOrder`）、`lazy`、`dataKey` 与分页相关插槽（见 design-spec §7）。规模超条目估算（原估 4–6 文件 / ~250 行，实际 24 文件 / 新增约 1.0k 行（含本条规划登记），其中 src 组件 9 文件 / 364 行），偏差因单一验收条目含组件 + locale 命名空间 + 中英文档与 3 个示例，且验收标准（API 文档 + 单测 + 英文文档页）要求同批完成；按 AI 协作规范 §3.2.1 分「组件核心」与「文档与示例」两批实现（单轮 Review Gate 覆盖两批，2 项 warning 已关闭）
