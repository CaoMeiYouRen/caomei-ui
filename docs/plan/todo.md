...
- 执行范围：先落治理规范（长期任务机制 + 公共逻辑抽取准则），再执行首轮抽取——标签属性转发（`label` 优先于透传 `aria-label`，10 处，含 `progress-bar` 等价形态）、ARIA 布尔假值归一（15 处）、attrs 透传收敛（手写 `useAttrs()` 12 文件）；其余候选（locale 文本解析 39 处、选项列表渲染 3 份、阴影 / 遮罩 token 13 处）转入 [长期任务台账](./recurring.md) 待后续轮次。
...
- 条目：
  - [ ] 治理规范落地（[长期任务台账](./recurring.md) + 规划规范长期任务制度 + 开发规范抽取准则）
  - [ ] 标签属性转发抽取（10 处，含 `progress-bar` 等价形态）
  - [ ] ARIA 布尔假值归一（15 处）
  - [ ] attrs 透传收敛（手写 `useAttrs()` 12 文件）
