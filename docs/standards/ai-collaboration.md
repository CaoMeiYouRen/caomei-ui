...
- 审计调用按 §3.1 声明 `audit-depth` 与时间盒；并发分区按 §3.2；用时核验按 §3.3。
...
| `deep` | 发布流程、安全 / 鉴权、外部调用、数据写入、运行时 / 发布配置与依赖版本变更、agent / skill 定义 | 全量 checklist + 针对性验证（临时仓库 / 本地实验 / 验证命令按需执行） | ≤ 20 分钟 |

调用方（`@full-stack-master` 或用户）发起审计时**必须声明**：

- `audit-depth`（`quick` / `standard` / `deep`）+ 理由；
- **时间盒**：单分区取该档位上限，并发分区按 §3.2 取值；复审按修复点规模重算，不超过上轮档位；
...
- 分区维度只有模块：单模块大改动不适用并发，按任务粒度约束拆批次。
