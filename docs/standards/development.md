...
- 受控数值输入：聚焦期间不因外部 model 变化回填文本，失焦与步进统一 `clamp(round(value))` 规范化；`step` 非正回退为 1，`precision` 取 0–20 的整数（与 `minFractionDigits` / `maxFractionDigits` 同域）、超出按未提供处理；取整用的 `10 ** n` 需做有限性守卫，极大值乘 factor 会溢出为 `Infinity` 并污染模型。
...
- 同一规则内不得重复声明同名属性（含自定义属性）：后写覆盖先写、先写者恒为死代码（调色 / 改值时易误改被覆盖的那条），`check:design` 的重复声明守卫 `[dup-decl]` 为预算 0，**无例外清单**。注意「先写安全值、后写 `color-mix()`」这类写法**不构成渐进增强回退**：后写声明含 `var()` 时不会被解析期丢弃，而是级联选中后在计算值期非法 → 置 `unset`（同下一条 IACVT 机制），故先写值在任何引擎都不会生效；需要渐进增强时应改用 `@supports (color: color-mix(in srgb, red, blue))` 或自定义属性中间变量 + 单条声明。规则面为**平铺规则**（`scanRules` 不展开 CSS 嵌套内部）。
...
- **抽取门槛**：同一模式在 ≥3 处出现且语义一致，并且抽取后净收益为正（一致性 / 可测性提升大于新增的间接层）；三项同时成立才实施。
- **不得为抽取而抽取**：仅「写法相似」而语义、契约或可访问名不同的模式不合并；未达门槛的候选按长期任务制度登记，不立即实施。
